...
class Gui(Tk):
   ...
   # Add Name image label
    ...
    def __add_animation_button(self):
        ...
        self.animation_button.grid(row=5, column=0, columnspan=2, padx=10)
        self.animation_button.configure(bg="#0ff", text="Start Animation", width=50, padx=5) 
    ...
    def __add_animation_frame(self):
        ...
        self.animation_frame.configure(bg="#B0E0E6", height=200, width=400, padx=15, pady=15)

    def __animation_button_clicked(self, event):
        animation = self.__animation_button_clicked
        name = self.name_entry.get()
        if animation != "" and name != "":
            self.name_image_label = Label(self.animation_frame)
            self.name_image_label.place(x = 20, y = 20)
            self.name_image_label.configure(image=self.unknown_image)
        if animation != "" and name == "Snowy":
            self.name_image_label = Label(self.animation_frame)
            self.name_image_label.place(x = 20, y = 20)
            self.name_image_label.configure(image=self.snowman_image)
